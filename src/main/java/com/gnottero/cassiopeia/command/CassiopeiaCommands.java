...
public class CassiopeiaCommands {
                ...
                dispatcher.register(Commands.literal("cassiopeia")
                                .then(Commands.literal("save")
                                                .then(Commands.argument("from", BlockPosArgument.blockPos())
                                                                .then(Commands.argument("to",
                                                                                ...
                                                                                .then(Commands.argument("controller",
                                                                                                BlockPosArgument.blockPos())
                                ...
                                .then(Commands.literal("verify")
                                                .then(Commands.argument("controller", BlockPosArgument.blockPos())
                                                                .then(Commands.argument("identifier",
                                                                                StringArgumentType.word())
                                                                                .suggests(SUGGEST_STRUCTURES)
                                                                                .executes(CassiopeiaCommands::executeVerify)))));
        }

        private static final com.mojang.brigadier.suggestion.SuggestionProvider<CommandSourceStack> SUGGEST_STRUCTURES = (
                        ctx,
                        builder) -> {
        ...
        private static int executeSave(CommandContext<CommandSourceStack> ctx, boolean keepAir)
                ...
                try {
                        StructureManager.saveStructure(ctx.getSource().getLevel(), from, to, controller, identifier,
                                        keepAir);
                        ...
                        ctx.getSource()
                                        .sendFailure(Component.translatable("command.cassiopeia.structure.save.failed",
                                                        e.getMessage()));
                        e.printStackTrace();
                        return 0;
                }
        ...
        private static int executeVerify(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
                ...
                if (structureOpt.isEmpty()) {
                        ctx.getSource().sendFailure(
                                        Component.translatable("command.cassiopeia.structure.not_found", identifier));
                        return 0;
                }
